...
minikube start --driver=hyperv --container-runtime=cri-o --cpus 4 --memory 1800 --disk-size '40000mb'

MSG_ERROR -step "starting minikube" -return_code $?	
echo "We stop the VM to activate dynamic memory"
minikube stop
Set-VMMemory minikube -DynamicMemoryEnabled $true -MinimumBytes 1800MB -StartupBytes 1800MB -MaximumBytes $(((((Get-CimInstance Win32_PhysicalMemory | Measure-Object -Property capacity -Sum).sum - 8gb) /1mb), 1800 | Measure -Max).Maximum * 1mb) -Priority 50 -Buffer 20
